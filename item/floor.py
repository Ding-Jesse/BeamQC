...
class Floor:
    ...
    def set_column_prop(self, kwargs):
        self.material_list.update({'fc': kwargs["混凝土強度fc'(kgf/cm2)"]})
        self.material_list.update({'fy': kwargs["鋼筋強度fy(kgf/cm2)"]})
        self.overlap_option.update(
            {"tight_tie": kwargs["全段緊密"], "coupler": kwargs["續接器"], "overlap": kwargs["續接方式"]})
        try:
            self.height = float(kwargs["樓高"])
        except:
            self.height = 0
    ...
    if item_type == 'column':
        for floor in floor_list:
            ...
            new_row_concrete = pd.DataFrame(
                floor.concrete_count, index=[floor.floor_name])
            new_row_coupler = pd.DataFrame(
                floor.coupler, index=[floor.floor_name])
            new_row_formwork = pd.DataFrame(
                {'模板': floor.formwork_count}, index=[floor.floor_name])

            df = pd.concat([df, new_row], verify_integrity=True)
            concrete_df = pd.concat(
                [concrete_df, new_row_concrete], verify_integrity=True)
            coupler_df = pd.concat(
                [coupler_df, new_row_coupler], verify_integrity=True)
...
def summary_floor_rebar_ratio(floor_list: list[Floor], beam_type: None):
    ...
    for floor in floor_list:
        if beam_type:
            beam_list = [
                beam for beam in floor.beam_list if beam.beam_type == beam_type]
        else:
    ...
    ratio_beam = pd.DataFrame(empty(
        [len(floor_list)*2, len(df_header_list)], dtype='<U16'), columns=df_header_list)
    ...
    for floor, ratio_dict in floor_dict.items():
        ...
        if len(floor_object.beam_list) == 0:
            continue
        ...
        for ratio, count_list in ratio_dict.items():
            for i, count in enumerate(count_list[:3]):
                ratio_beam.at[row, (ratio, pos[i])] = round(
                    count / len(floor_object.beam_list), 2)
            for i, count in enumerate(count_list[3:]):
                ratio_beam.at[row + 1, (ratio, pos[i])
                              ] = round(count / len(floor_object.beam_list), 2)
        row += 2
...
def summary_floor_column_rebar_ratio(floor_list: list[Floor]):
    ...
    header_list = list(map(
        lambda r, p: f'{round(p*100,2)}% ≤ 鋼筋比 < {round(r*100,2)}%', ratio_upper_bound_group, ratio_lower_bound_group))
    header_list.append(f'≥ {ratio_upper_bound_group[-1]*100}%')
    ...
    for floor in floor_list:
        ...
        for column in floor.column_list:
            if column.x_size * column.y_size == 0:
                continue
