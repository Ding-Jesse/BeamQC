            ...
            try:
                ...
                # 抓箭頭座標
                    ...
                    if hasattr(object,'startPoint'):
                        coor_to_arrow_dic[(round(object.startPoint[0], 2), round(object.startPoint[1], 2))] = (round(object.endPoint[0], 2), round(object.endPoint[1], 2))
                # 抓鋼筋本人和箍筋本人
            ...
            for key,value in {k:v for k,v in arrow_dict.items() if abs(k[1] - value_pair[0][1]) < min_diff}.items():
                ...
                if dim_list:
                    ...
                    if abs(text_postion[1] - key[1]) < 150:
                        rebar_coor1 = ((line_1_point[0] + line_2_point[0])/2,head_coor[1])
                        rebar_coor2 = (value[0],value[1])
                        length = text_value
                        with_dim = True
...
def sort_arrow_to_word(coor_to_arrow_dic:dict,
    ...
    for arrow_head,arrow_data in coor_to_arrow_dic.items():
        ...
        if arrow_head[1] > arrow_tail[1]:
            ## 箭頭朝上
            rebar_data_temp = [r for r in coor_to_data_list if '@' not in r[0] and r[1][1] <= arrow_head[1]]
        if arrow_head[1] < arrow_tail[1]:
            ## 箭頭朝下
            rebar_data_temp = [r for r in coor_to_data_list if '@' not in r[0] and r[1][1] >= arrow_head[1]]
        if not rebar_data_temp:
            progress(f'arrow head:{arrow_head} has no rebar data', progress_file)
        ...
        if(abs(arrow_tail[1] - coor[1])> min_diff):
            progress(f'{arrow_head} / {arrow_data} cant find pair arrow', progress_file)
        ...
        head_to_data_dic.update({(line_coor[0] - rebar_length/2,line_coor[1]):{'number':number,'size':size , 'dim':with_dim}})
        tail_to_data_dic.update({(line_coor[0] + rebar_length/2,line_coor[1]):{'number':number,'size':size , 'dim':with_dim}})
        ...
        for line in line_list[:]:
            ...
            if (not head_rebar) and (not tail_rebar):
                ...
                coor_to_rebar_list_straight.append((head_coor, tail_coor,
                                                    _cal_length(head_coor,tail_coor),
...
def count_tie(coor_to_tie_text_list:list,coor_to_block_list:list,coor_to_tie_list):
    ...
    for tie,coor in coor_to_tie_text_list: # (字串，座標)
        ...
        if '-' in tie:
            ...
            tie_left_list = [(bottom,top,length) for bottom,top,length in coor_to_tie_list if ( bottom[0] < coor[0]) and (min(bottom[1],top[1]) < coor[1]) and (max(bottom[1],top[1]) > coor[1])]
            tie_right_list = [(bottom,top,length) for bottom,top,length in coor_to_tie_list if ( bottom[0] > coor[0]) and (min(bottom[1],top[1]) < coor[1]) and (max(bottom[1],top[1]) > coor[1])]
...
def combine_beam_boundingbox(coor_to_block_list:list[tuple[tuple[tuple,tuple],dict,dict]],
    ...
    for beam in class_beam_list:
        if beam.beam_type == BeamType.FB:
            ...
            accept_y_diff = 0
        ## 先找圖框
        outer_block = [block for block in coor_to_block_list if inblock(block[0],beam.get_coor())]
        bounding_box = [block for block in coor_to_bounding_block_list if inblock(block[0],beam.get_coor())]
        if len(bounding_box)==0:
            if outer_block:
                same_block_rc_block_list = [block for block in coor_to_rc_block_list if inblock(outer_block[0][0],block[0][0])]

            left_bounding_box_list = [block for block in same_block_rc_block_list if block[0][1][0] <= beam.get_coor()[0] and block[0][1][1] > beam.get_coor()[1] - accept_y_diff]
            right_bounding_box_list = [block for block in same_block_rc_block_list if block[0][0][0] >= beam.get_coor()[0] and block[0][0][1] > beam.get_coor()[1] - accept_y_diff]
            if len(left_bounding_box_list) and len(right_bounding_box_list):
                ...
                if abs(beam.get_coor()[1] - max(top_bounding,bot_bounding)) < 100:
                    left_bounding_box_list = [new_block for new_block in left_bounding_box_list if new_block[0][1][1] > top_bounding + 15]
                    right_bounding_box_list = [new_block for new_block in right_bounding_box_list if new_block[0][1][1] > top_bounding + 15]
                    new_top_bound = []
                    if not len(left_bounding_box_list) and not len(right_bounding_box_list):
                        print(f'{beam.floor}{beam.serial} no bounding, use outer block')
                    ...
                    if left_bounding_box_list:
                        ...
                        new_top_bound.append(max(new_left_bounding_box[0][0][1],new_left_bounding_box[0][1][1]))
                    if right_bounding_box_list:
                        ...
                        new_top_bound.append(max(new_right_bounding_box[0][0][1],new_right_bounding_box[0][1][1]))
...
def break_down_rebar(coor_to_arrow_dic:dict,class_beam_list:list[Beam]):
    ...
    for arrow_head in list(coor_to_arrow_dic.keys()):
        ...
        left_bounding_box = [beam for beam in class_beam_list if inblock(block=beam.get_bounding_box(),pt=line_left_pt)]
        if len(left_bounding_box) > 1:
            left_nearest_beam = min(left_bounding_box,key=lambda b:_get_distance(b.get_coor(),line_left_pt))
        elif left_bounding_box:
            left_nearest_beam = left_bounding_box[0]
        
        right_bounding_box = [beam for beam in class_beam_list if inblock(block=beam.get_bounding_box(),pt=line_right_pt)]
        if len(right_bounding_box) > 1:
            right_nearest_beam = min(right_bounding_box,key=lambda b:_get_distance(b.get_coor(),line_right_pt))
        ...
        if nearest_beam.serial and left_nearest_beam and right_nearest_beam:
            ...
            if (nearest_beam.serial != left_nearest_beam.serial or 
                ...
                coor_to_arrow_dic.update({arrow_head:((new_left_pt_x,line_left_pt[1]),
                                                        abs(new_left_pt_x - new_right_pt_x),
                                                        new_mid_coor,
                                                        False,
                                                        number,
                                                        size,
...
def create_scan_report(floor_list:list[Floor],
    ...
    OutputExcel(df_list=[code_df,enoc_df],file_path=excel_filename,
                ...
                rows_list=range(2,len(code_df.index)+ 10 + len(enoc_df.index)),
                df_spacing= 3)
    Add_Row_Title(file_path=excel_filename,sheet_name=f'{item_name}檢核表',
                  i=len(code_df.index) + 4,j=1,
                  title_text='經濟性檢核',font_size= 20)
    OutputExcel(df_list=[ng_df],file_path=excel_filename,sheet_name=f'{item_name}詳細檢核表')
    OutputExcel(df_list=[rebar_df],file_path=excel_filename,sheet_name=f'{item_name}鋼筋統計表')
    OutputExcel(df_list=[ratio_df],
            file_path=excel_filename,
...
def draw_rebar_line(class_beam_list:list[Beam],msp_beam:object,doc_beam:object,output_folder:str,project_name:str):
    ...
    output_dwg = os.path.join(output_folder,f'{project_name}_{date}_Markon.dwg')
    while error_count <= 10:
        try:
    ...
    for b in class_beam_list:
        try:
            ...
            if b.rebar_table['top_length'][RebarType.Left.value]:
                beam.at[row,('主筋長度', '左')] = b.rebar_table['top_length'][RebarType.Left.value][0]
            if b.rebar_table['top_length'][RebarType.Middle.value]:
                beam.at[row,('主筋長度', '中')] = b.rebar_table['top_length'][RebarType.Middle.value][0]
            if b.rebar_table['top_length'][RebarType.Right.value]:
                beam.at[row,('主筋長度', '右')] = b.rebar_table['top_length'][RebarType.Right.value][0]
            if b.rebar_table['bottom_length'][RebarType.Left.value]:
                beam.at[row + 3,('主筋長度', '左')] = b.rebar_table['bottom_length'][RebarType.Left.value][0]
            if b.rebar_table['bottom_length'][RebarType.Middle.value]:
                beam.at[row + 3,('主筋長度', '中')] = b.rebar_table['bottom_length'][RebarType.Middle.value][0]
            if b.rebar_table['bottom_length'][RebarType.Right.value]:
                beam.at[row + 3,('主筋長度', '右')] = b.rebar_table['bottom_length'][RebarType.Right.value][0]
...
def output_rcad_beam(class_beam_list:list[Beam]):
    ...
    header_info_1 = [('樓層', ''), ('梁編號', ''),('型式', ''), ('梁尺寸', '樑寬B '), ('梁尺寸', '樑深H '),
                     ('上層筋','左'),('上層筋','中'),('上層筋','右'),('下層筋','左'),('下層筋','中'),('下層筋','右'),
                     ('腰筋','單側'),('箍筋','左'),('箍筋','中'),('箍筋','右'),('垂直繫筋',''),
                     ('左斷筋點','lcut_neg'),('左斷筋點','lcut_pos'),('右斷筋點','lcut_neg'),('右斷筋點','lcut_pos')]
    header = pd.MultiIndex.from_tuples(header_info_1)
    rcad_beam = pd.DataFrame(np.empty([len(class_beam_list),len(header)],dtype='<U16'),columns=header)
    row = 1
    for b in class_beam_list:
        left_neg = 0
        left_pos = 0
        ...
        rcad_beam.at[row,('上層筋','左')] = ','.join(list(map(lambda r:r.text.replace('-','') ,b.rebar_table[RebarType.Top.value][RebarType.Left.value])))
        rcad_beam.at[row,('上層筋','中')] = ','.join(list(map(lambda r:r.text.replace('-','') ,b.rebar_table[RebarType.Top.value][RebarType.Middle.value])))
        rcad_beam.at[row,('上層筋','右')] = ','.join(list(map(lambda r:r.text.replace('-','') ,b.rebar_table[RebarType.Top.value][RebarType.Right.value])))
        rcad_beam.at[row,('下層筋','左')] = ','.join(list(map(lambda r:r.text.replace('-','') ,b.rebar_table[RebarType.Bottom.value][RebarType.Left.value])))
        rcad_beam.at[row,('下層筋','中')] = ','.join(list(map(lambda r:r.text.replace('-','') ,b.rebar_table[RebarType.Bottom.value][RebarType.Middle.value])))
        rcad_beam.at[row,('下層筋','右')] = ','.join(list(map(lambda r:r.text.replace('-','') ,b.rebar_table[RebarType.Bottom.value][RebarType.Right.value])))
        rcad_beam.at[row,('腰筋','單側')] = b.middle_tie[0].text.replace('-','').replace('E.F','') if b.middle_tie else '0'
        rcad_beam.at[row,('箍筋','左')] = b.tie[RebarType.Left.value].text if b.tie[RebarType.Left.value] else ''
        rcad_beam.at[row,('箍筋','中')] = b.tie[RebarType.Middle.value].text if b.tie[RebarType.Middle.value] else ''
        rcad_beam.at[row,('箍筋','右')] = b.tie[RebarType.Right.value].text if b.tie[RebarType.Right.value] else ''
        if b.rebar_table["top_length"][RebarType.Left.value]:
            left_neg = ceil(b.rebar_table["top_length"][RebarType.Left.value][0])
        if b.rebar_table["bottom_length"][RebarType.Left.value]:
            left_pos = ceil(b.rebar_table["bottom_length"][RebarType.Left.value][0])
        if b.rebar_table["top_length"][RebarType.Right.value]:
            right_neg = ceil(b.rebar_table["top_length"][RebarType.Right.value][0])
        if b.rebar_table["bottom_length"][RebarType.Right.value]:
            right_pos = ceil(b.rebar_table["bottom_length"][RebarType.Right.value][0])
...
def count_beam_multiprocessing(beam_filenames:list,layer_config:dict,temp_file='temp_1221_1F.pkl',output_folder='',project_name='',template_name='',floor_parameter_xlsx = ''):
    ...
    excel_filename,excel_filename_rcad = create_report(class_beam_list=beam_list,
...
def compare_line_with_dim(coor_to_dim_list:list[tuple[tuple[float,float],float,tuple[tuple[float,float],tuple[float,float]]]],
    ...
    if True:
        for block in coor_to_block_list:
            dim_list = [dim for dim in coor_to_dim_list if inblock(block[0],dim[0])]
                ...
                for rebar in beam.rebar_list:
                    try:
                        ...
                        if abs(rebar.length - min_dim[1]) > 1:
                            ...
                            if abs(rebar.length - min_dim[1]) < 1:continue 
                        ...
                        print(f'{beam.floor}:{beam.serial} compare_line_with_dim error')
...
if __name__=='__main__':
    ...
    class_beam_list = save_temp_file.read_temp(r'D:\Desktop\BeamQC\TEST\2023-0607\0607-GB-list.pkl')
    # class_beam_list.extend(save_temp_file.read_temp(r'D:\Desktop\BeamQC\TEST\2023-0607\0607-FB-list.pkl'))
    cad_data = save_temp_file.read_temp(r'D:\Desktop\BeamQC\TEST\2023-0607\0607-GB-cad.pkl')
    ...
    # data2=save_temp_file.read_temp(r'D:\Desktop\BeamQC\temp_0224_sb_fb_b-1.pkl')
    # print('')
    # import pprint
